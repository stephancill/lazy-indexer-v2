...
import { scheduleBackfillJob, getAllQueueStats, clearQueue } from '@farcaster-indexer/indexer';
...
export const backfillCommand = new Command('backfill')
    ...
    new Command('start')
      ...
      .action(async (options) => {
        try {
          ...
          logger.updateSpinner(`Queuing ${targetsToBackfill.length} backfill jobs...`);
          
          for (const target of targetsToBackfill) {
            await scheduleBackfillJob(target.fid, target.isRoot);
          }
          
          logger.stopSpinner(true, `Queued ${targetsToBackfill.length} backfill jobs successfully`);
          
          if (targetsToBackfill.length > 0) {
            logger.info(`Jobs queued for FIDs: ${targetsToBackfill.map(t => t.fid).join(', ')}`);
          }
        } catch (error) {
      ...
      .action(async () => {
        try {
          ...
          if (backfillStats.failed > 0) {
            logger.warn(`${backfillStats.failed} failed jobs. Use 'jobs failed' to see details.`);
          }
        } catch (error) {
      ...
      .action(async (options) => {
        try {
          ...
          if (options.fid) {
            ...
            logger.stopSpinner(true, 'Cleared failed jobs - they can now be rescheduled');
          } else {
            logger.error('Specify --all to retry all failed jobs or --fid <fid> to retry specific FID');
            process.exit(1);
          }
        } catch (error) {
  ...
  .addCommand(
    new Command('clear')
      ...
      .action(async (options) => {
        if (!options.confirm) {
          logger.error('This operation will clear completed jobs. Use --confirm to proceed.');
          process.exit(1);
        }
        ...
        try {
          ...
          logger.stopSpinner(true, 'Completed backfill jobs cleared successfully');
        } catch (error) {
