...
export const targetCommands = new Command("targets")
  ...
  .addCommand(
    new Command("list")
      ...
      .option(
        "--sort <field>",
        "Sort by field (fid, added_at, last_synced_at)",
        "fid"
      )
      ...
      .action(async (options) => {
        try {
          ...
          const sortField =
            options.sort === "added_at"
              ? targets.addedAt
              : options.sort === "last_synced_at"
                ? targets.lastSyncedAt
                : targets.fid;
          ...
          if (options.json) {
            ...
            logger.table(
              results.map((t) => ({
                ...
                "Last Synced": t.lastSyncedAt
                  ? t.lastSyncedAt.toISOString().split("T")[0]
                  : "Never",
              }))
            );
          }
  ...
  .addCommand(
    new Command("remove")
      ...
      .action(async (fid, options) => {
        if (!options.confirm) {
          logger.error(
            "This operation will remove the target. Use --confirm to proceed."
          );
          process.exit(1);
        }
      ...
      .action(async () => {
        try {
          ...
          const [totalTargets] = await db
            .select({ count: count() })
            .from(targets);
          const [rootTargets] = await db
            .select({ count: count() })
            .from(targets)
            .where(eq(targets.isRoot, true));
          const [unsyncedTargets] = await db
            .select({ count: count() })
            .from(targets)
            .where(isNull(targets.lastSyncedAt));
          const [syncedTargets] = await db
            .select({ count: count() })
            .from(targets)
            .where(isNotNull(targets.lastSyncedAt));
        ...
        new Command("list")
          ...
          .action(async (options) => {
            try {
              ...
              logger.stopSpinner(
                true,
                `Found ${results.length} client targets`
              );
        ...
        new Command("add")
          ...
          .action(async (fid) => {
            try {
              ...
              logger.stopSpinner(
                true,
                `Client target ${fidNum} added successfully`
              );
            } catch (error) {
      ...
      .addCommand(
        new Command("remove")
          ...
          .action(async (fid, options) => {
            if (!options.confirm) {
              logger.error(
                "This operation will remove the client target. Use --confirm to proceed."
              );
              process.exit(1);
            }
            ...
            try {
              ...
              logger.stopSpinner(
                true,
                `Client target ${fidNum} removed successfully`
              );
            } catch (error) {
