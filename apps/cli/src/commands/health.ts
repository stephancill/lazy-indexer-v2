      ...
      .action(async (options) => {
        try {
          ...
          try {
            ...
            details.push({ component: 'Database', status: 'healthy', message: 'Connection successful' });
          } catch (error) {
            details.push({ 
              component: 'Database', 
              status: 'unhealthy', 
          ...
          try {
            ...
            details.push({ component: 'Redis', status: 'healthy', message: 'Connection successful' });
          } catch (error) {
            details.push({ 
              component: 'Redis', 
              status: 'unhealthy', 
          ...
          try {
            ...
            const backfillStats = queueStats.find(q => q.queue === 'backfill');
            const realtimeStats = queueStats.find(q => q.queue === 'realtime');
            
            if (backfillStats) {
              healthStatus.queues.backfill = true;
              details.push({ component: 'Backfill Queue', status: 'healthy', message: 'Queue accessible' });
            } else {
              details.push({ component: 'Backfill Queue', status: 'unhealthy', message: 'Queue not found' });
            }
            
            if (realtimeStats) {
              healthStatus.queues.realtime = true;
              details.push({ component: 'Realtime Queue', status: 'healthy', message: 'Queue accessible' });
            } else {
              details.push({ component: 'Realtime Queue', status: 'unhealthy', message: 'Queue not found' });
            }
          } catch (error) {
            details.push({ 
          ...
          // Overall health
          healthStatus.overall = healthStatus.database && 
                                healthStatus.redis && 
                                healthStatus.hubClient && 
                                healthStatus.queues.backfill && 
                                healthStatus.queues.realtime;
          
          logger.stopSpinner(true, `Health check completed - ${healthStatus.overall ? 'System healthy' : 'Issues detected'}`);
          
          if (options.json) {
            logger.json({ status: healthStatus, details });
          } else {
            logger.table(details.map(detail => ({
              Component: detail.component,
              Status: detail.status === 'healthy' ? '✅ Healthy' : '❌ Unhealthy',
    ...
    new Command('database')
      ...
      .action(async () => {
        try {
          ...
          const stats = [];
          try {
            const targetsCount = await db.execute(`SELECT COUNT(*) as count FROM targets`);
            stats.push({ table: 'targets', count: targetsCount[0]?.count || 0 });
            
            const usersCount = await db.execute(`SELECT COUNT(*) as count FROM users`);
            stats.push({ table: 'users', count: usersCount[0]?.count || 0 });
            
            const castsCount = await db.execute(`SELECT COUNT(*) as count FROM casts`);
            stats.push({ table: 'casts', count: castsCount[0]?.count || 0 });
            
            const reactionsCount = await db.execute(`SELECT COUNT(*) as count FROM reactions`);
            stats.push({ table: 'reactions', count: reactionsCount[0]?.count || 0 });
            
            const linksCount = await db.execute(`SELECT COUNT(*) as count FROM links`);
          ...
          logger.info(`Migrations table: ${migrationsExist ? 'Exists' : 'Not found'}`);
          
          if (stats.length > 0) {
            logger.line();
            logger.table(stats.map(stat => ({
              Table: stat.table,
          ...
          logger.error(`Database error: ${error instanceof Error ? error.message : 'Unknown error'}`);
          process.exit(1);
        }
      })
  )
    ...
    new Command('redis')
      ...
      .action(async () => {
        try {
          ...
          logger.info(`Uptime: ${uptime ? Math.floor(parseInt(uptime) / 86400) + ' days' : 'Unknown'}`);
          ...
          logger.info(`Client target set exists: ${clientSetExists ? 'Yes' : 'No'}`);
        } catch (error) {
    ...
    new Command('hubs')
      ...
      .action(async () => {
        try {
          ...
          const healthyHubs = results.filter(r => r.status === 'healthy').length;
          ...
          logger.error(`Hub error: ${error instanceof Error ? error.message : 'Unknown error'}`);
          process.exit(1);
        }
      })
  );