...
export const exportCommand = new Command("export")
  ...
  .option(
    "--table <table>",
    "Specific table to export (targets, users, casts, reactions, links, verifications)",
    "all"
  )
  .option(
    "--fids <fids>",
    "Comma-separated list of FIDs to export (for user-specific data)"
  )
  ...
  .action(async (options) => {
    try {
      ...
      if (options.table === "all" || options.table === "target_clients") {
        ...
        let query = db.select().from(targetClients).$dynamic();
        if (limit) {
          query = query.limit(limit);
        }
      ...
      if (options.table === "all" || options.table === "casts") {
        ...
        let query = db.select().from(casts).$dynamic();
        if (fidList && fidList.length > 0) {
      ...
      if (options.table === "all" || options.table === "reactions") {
        ...
        let query = db.select().from(reactions).$dynamic();
        if (fidList && fidList.length > 0) {
      ...
      if (options.table === "all" || options.table === "links") {
        ...
        let query = db.select().from(links).$dynamic();
        if (fidList && fidList.length > 0) {
      ...
      if (options.table === "all" || options.table === "verifications") {
        ...
        let query = db.select().from(verifications).$dynamic();
        if (fidList && fidList.length > 0) {
      ...
      logger.stopSpinner(
        true,
        `Exported ${totalRecords} records to ${options.output}`
      );

      // Show export summary
      const summary = Object.entries(exportData.tables).map(
        ([table, data]) => ({
          Table: table,
...
export const importCommand = new Command("import")
  ...
  .option(
    "--table <table>",
    "Specific table to import (targets, users, casts, reactions, links, verifications)",
    "all"
  )
  ...
  .action(async (file, options) => {
    try {
      ...
      if (!importData.tables || typeof importData.tables !== "object") {
        logger.error(
          'Invalid import file format. Expected JSON with "tables" object.'
        );
        process.exit(1);
      }
      ...
      // Import targets
      if (
        (options.table === "all" || options.table === "targets") &&
        importData.tables.targets
      ) {
      ...
      if (
        (options.table === "all" || options.table === "target_clients") &&
        importData.tables.target_clients
      ) {
      ...
      // Import casts
      if (
        (options.table === "all" || options.table === "casts") &&
        importData.tables.casts
      ) {
      ...
      // Import reactions
      if (
        (options.table === "all" || options.table === "reactions") &&
        importData.tables.reactions
      ) {
      ...
      // Import links
      if (
        (options.table === "all" || options.table === "links") &&
        importData.tables.links
      ) {
      ...
      if (
        (options.table === "all" || options.table === "verifications") &&
        importData.tables.verifications
      ) {
      ...
      if (options.dryRun) {
        ...
        logger.stopSpinner(
          true,
          `Imported ${totalRecords} records successfully`
        );
      }
      ...
      if (options.replace && !options.dryRun) {
        logger.warn(
          "Data was replaced. Previous records in imported tables were deleted."
        );
      }
    } catch (error) {
