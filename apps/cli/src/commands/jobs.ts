...
import { getAllQueueStats, getQueueStats, pauseQueue, resumeQueue, clearQueue } from '@farcaster-indexer/indexer';
...
export const jobCommands = new Command('jobs')
  ...
  .addCommand(
    new Command('status')
      ...
      .option('--queue <queue>', 'Show specific queue (backfill, realtime)', 'all')
      ...
      .action(async (options) => {
        try {
          ...
          if (options.json) {
            ...
            logger.table(queueStats.map(stat => ({
              Queue: stat.queue,
    ...
    new Command('clear')
      ...
      .action(async (queue, options) => {
        ...
        try {
          ...
          logger.stopSpinner(true, `Jobs cleared from ${queue} queue successfully`);
        } catch (error) {
          logger.stopSpinner(false, `Failed to clear completed jobs from ${queue} queue`);
          logger.error(`Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
          process.exit(1);
        }
      })
