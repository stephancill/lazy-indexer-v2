...
export const debugCommand = new Command("debug")
      ...
      .action(async (fid, options) => {
        try {
          ...
          const [reactionsCount] = await db
            .select({ count: count() })
            .from(reactions)
            .where(eq(reactions.fid, fidNum));
          const [linksCount] = await db
            .select({ count: count() })
            .from(links)
            .where(eq(links.fid, fidNum));
          const [verificationsCount] = await db
            .select({ count: count() })
            .from(verifications)
            .where(eq(verifications.fid, fidNum));
          ...
          if (options.json) {
            ...
            logger.info(
              `Last Synced: ${
                target.lastSyncedAt
                  ? target.lastSyncedAt.toISOString()
                  : "Never"
              }`
            );
            ...
            if (userInfo.length > 0) {
              ...
              logger.info(
                `User Synced: ${
                  user.syncedAt ? user.syncedAt.toISOString() : "Never"
                }`
              );
            } else {
  ...
  .addCommand(
    new Command("hub")
      ...
      .option(
        "--type <type>",
        "Data type to fetch (casts, reactions, links, verifications, user)",
        "user"
      )
      .option("--json", "Output as JSON")
      .action(async (fid, options) => {
        try {
          ...
          data = {
            message: `Debug for ${options.type} not yet available`,
            fid: fidNum,
          };
          ...
          if (options.json) {
            ...
            logger.info(
              `Results: ${Array.isArray(data) ? data.length : 1} items`
            );
            logger.line();
          ...
          logger.stopSpinner(
            false,
            `Failed to fetch ${options.type} for FID ${fid}`
          );
          logger.error(
            `Error: ${error instanceof Error ? error.message : "Unknown error"}`
          );
          ...
          if (options.key) {
            ...
            switch (type) {
              ...
              case "list":
                ...
                break;
              case "zset":
                value = await redisClient.zrange(
                  options.key,
                  0,
                  -1,
                  "WITHSCORES"
