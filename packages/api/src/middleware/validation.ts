...
export const validateQueueParam = async (c: Context, next: Next) => {
  ...
  if (queueName === null) {
    return c.json({ error: 'Invalid queue name. Must be one of: backfill, realtime, process-event' }, 400);
  }
...
export const validateTargetBody = (body: any): { fid: number; isRoot?: boolean } | string => {
...
export const validateClientTargetBody = (body: any): { clientFid: number } | string => {
  if (!body || typeof body !== 'object') {
    return 'Request body is required';
  }
...
export const validateTargetUpdateBody = (body: any): { isRoot?: boolean } | string => {
  if (!body || typeof body !== 'object') {
    return 'Request body is required';
  }
