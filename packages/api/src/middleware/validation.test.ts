...
describe("Validation Functions", () => {
  ...
  describe("validateTargetBody", () => {
    ...
    it("should handle various isRoot values", () => {
      expect(validateTargetBody({ fid: 123, isRoot: true })).toEqual({
        fid: 123,
        isRoot: true,
      });
      expect(validateTargetBody({ fid: 123, isRoot: false })).toEqual({
        fid: 123,
        isRoot: false,
      });
      expect(validateTargetBody({ fid: 123, isRoot: "true" })).toEqual({
      ...
      expect(validateTargetBody({ fid: 123, isRoot: 1 })).toEqual({
        fid: 123,
        isRoot: false,
      }); // Only true boolean
    });
  ...
  describe("validateClientTargetBody", () => {
    ...
    it("should return error for invalid clientFid", () => {
      ...
      expect(validateClientTargetBody({ clientFid: 0 })).toBe(
        "Valid client FID is required"
      );
      expect(validateClientTargetBody({ clientFid: -1 })).toBe(
        "Valid client FID is required"
      );
      expect(validateClientTargetBody({ clientFid: "abc" })).toBe(
        "Valid client FID is required"
      );
    });
  });
  ...
  describe("validateTargetUpdateBody", () => {
    ...
    it("should return error for non-boolean isRoot", () => {
      expect(validateTargetUpdateBody({ isRoot: "true" })).toBe(
        "isRoot must be a boolean value"
      );
      expect(validateTargetUpdateBody({ isRoot: 1 })).toBe(
        "isRoot must be a boolean value"
      );
      expect(validateTargetUpdateBody({ isRoot: "false" })).toBe(
        "isRoot must be a boolean value"
      );
    });
