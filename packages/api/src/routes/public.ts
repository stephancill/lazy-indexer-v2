...
publicRoutes.get('/casts/:hash', async (c) => {
  try {
    ...
    const [likesCount, recastsCount, repliesCount] = await Promise.all([
      ...
      db.select({ count: sql<number>`count(*)` })
        .from(reactions)
        .where(and(eq(reactions.targetHash, hash), eq(reactions.type, 'recast'))),
      db.select({ count: sql<number>`count(*)` })
        .from(casts)
...
publicRoutes.get('/feed/:fid', async (c) => {
  try {
    ...
    const following = await db.select({ targetFid: links.targetFid })
      .from(links)
      .where(and(eq(links.fid, fid), eq(links.type, 'follow')));
...
publicRoutes.get('/trending', async (c) => {
  try {
    ...
    const trendingCasts = await db.select({
      hash: casts.hash,
      fid: casts.fid,
      text: casts.text,
    ...
    const enrichedCasts = trendingCasts.map(trendingCast => {
      const castWithUser = castsWithUsers.find(c => c.hash === trendingCast.hash);
      return {
        ...trendingCast,
