...
vi.mock('../middleware/validation.js', () => ({
  ...
  validateClientTargetBody: vi.fn((body) => {
    ...
    if (isNaN(clientFid) || clientFid <= 0) return 'Valid client FID is required';
    return { clientFid };
  }),
...
describe('Enhanced Admin Routes', () => {
  ...
  describe('Enhanced Target Management', () => {
    it('should support advanced filtering with all parameters', async () => {
      ...
      const response = await app.request('/admin/targets?search=1&is_root=true&sync_status=synced&sort_by=fid&sort_order=asc&date_from=2024-01-01&date_to=2024-12-31');
    ...
    it('should validate date range parameters', async () => {
      const response = await app.request('/admin/targets?date_from=2024-12-31&date_to=2024-01-01');
    ...
    it('should add target with Redis cache integration', async () => {
      ...
      mockDb.db.insert.mockReturnValue({
        returning: vi.fn().mockResolvedValue([{ fid: 123, isRoot: false, addedAt: new Date() }])
      });
  ...
  describe('Job Management Integration', () => {
    it('should return real-time queue statistics', async () => {
      ...
      const mockStats = [
        { queue: 'backfill', active: 2, waiting: 5, completed: 100, failed: 1, delayed: 0, paused: 0 },
        { queue: 'realtime', active: 1, waiting: 0, completed: 50, failed: 0, delayed: 0, paused: 0 },
        { queue: 'process-event', active: 3, waiting: 10, completed: 200, failed: 2, delayed: 1, paused: 0 },
      ];
  ...
  describe('Client Target Management', () => {
    it('should add client target with Redis cache integration', async () => {
      ...
      mockDb.db.insert.mockReturnValue({
        returning: vi.fn().mockResolvedValue([{ clientFid: 456, addedAt: new Date() }])
      });
    ...
    it('should remove client target with Redis cache cleanup', async () => {
      ...
      mockDb.db.query.targetClients.findFirst.mockResolvedValue({ clientFid: 456 });
      mockDb.db.delete.mockReturnValue({ where: vi.fn() });
  ...
  describe('Error Handling', () => {
    it('should handle database errors gracefully', async () => {
      const mockDb = await vi.importMock('@farcaster-indexer/shared');
      mockDb.db.query.targets.findMany.mockRejectedValue(new Error('Database error'));
