...
describe('Full System Integration Tests', () => {
  ...
  describe('End-to-End Data Flow', () => {
    it('should process a complete user lifecycle', async () => {
      ...
      const target = await db.select().from(targets).where(eq(targets.fid, testFid)).limit(1).then(rows => rows[0]);
      ...
      const cast = await db.select().from(casts).where(eq(casts.hash, 'integration-test-hash')).limit(1).then(rows => rows[0]);
    ...
    it('should handle follow relationships and target expansion', async () => {
      ...
      const follow = await db.select().from(links).where(eq(links.hash, 'follow-hash')).limit(1).then(rows => rows[0]);
      ...
      const newTarget = await db.select().from(targets).where(eq(targets.fid, targetFid)).limit(1).then(rows => rows[0]);
    ...
    it('should handle reaction events to target casts', async () => {
      ...
      const reaction = await db.select().from(reactions).where(eq(reactions.hash, 'reaction-hash')).limit(1).then(rows => rows[0]);
    ...
    it('should handle client discovery events', async () => {
      ...
      const newTarget = await db.select().from(targets).where(eq(targets.fid, newUserFid)).limit(1).then(rows => rows[0]);
  ...
  describe('System Recovery and Resilience', () => {
    it('should recover from job failures gracefully', async () => {
      ...
      await expect(backfillWorker.processJob(testJob as any)).resolves.not.toThrow();
    });
    ...
    it('should handle database connection issues', async () => {
      ...
      await expect(processorWorker.processJob(processorJob as any)).resolves.not.toThrow();
    });
  });
