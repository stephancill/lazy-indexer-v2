...
describe('Load Testing', () => {
  ...
  describe('High-Volume Event Processing', () => {
    it('should handle 1000 cast events efficiently', async () => {
      ...
      console.log(`Processed ${numEvents} events in ${processingTime}ms (${eventsPerSecond.toFixed(2)} events/second)`);
    ...
    it('should handle 500 reaction events efficiently', async () => {
      ...
      const eventsPerSecond = numEvents / (processingTime / 1000);
      console.log(`Processed ${numEvents} reactions in ${processingTime}ms (${eventsPerSecond.toFixed(2)} reactions/second)`);
    ...
    it('should handle 200 follow events efficiently', async () => {
      ...
      const eventsPerSecond = numEvents / (processingTime / 1000);
      console.log(`Processed ${numEvents} follows in ${processingTime}ms (${eventsPerSecond.toFixed(2)} follows/second)`);
  ...
  describe('Database Performance', () => {
    it('should handle bulk inserts efficiently', async () => {
      ...
      console.log(`Inserted ${numTargets} targets in ${insertTime}ms (${insertsPerSecond.toFixed(2)} inserts/second)`);
      
      // Should handle at least 200 inserts per second
      expect(insertsPerSecond).toBeGreaterThan(200);
      
      // Verify all targets were inserted
      const count = await db.select().from(targets).then(rows => rows.length);
  ...
  describe('Concurrent Processing', () => {
    it('should handle multiple concurrent workers', async () => {
      ...
      const workers = Array.from({ length: numWorkers }, () => new ProcessorWorker());
      ...
      console.log(`${numWorkers} workers processed ${totalEvents} events in ${processingTime}ms (${eventsPerSecond.toFixed(2)} events/second)`);
