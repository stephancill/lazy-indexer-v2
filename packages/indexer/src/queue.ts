...
// Create queues
export const backfillQueue = new Queue<BackfillJobData>('backfill', queueConfig);
export const realtimeQueue = new Queue<RealtimeSyncJobData>('realtime', queueConfig);
export const processEventQueue = new Queue<ProcessEventJobData>('process-event', queueConfig);

// Queue events for monitoring
export const backfillQueueEvents = new QueueEvents('backfill', { connection: redisConnection });
export const realtimeQueueEvents = new QueueEvents('realtime', { connection: redisConnection });
export const processEventQueueEvents = new QueueEvents('process-event', { connection: redisConnection });
...
export function createBackfillWorker(processor: (job: BackfillJob) => Promise<void>) {
...
export function createRealtimeWorker(processor: (job: RealtimeSyncJob) => Promise<void>) {
...
export function createProcessEventWorker(processor: (job: ProcessEventJob) => Promise<void>) {
...
export async function scheduleBackfillJob(fid: number, isRoot: boolean, delay = 0) {
  const jobData: BackfillJobData = { fid, isRoot };
  
  return backfillQueue.add(
    `backfill-${fid}`,
    jobData,
    {
      delay,
...
export async function scheduleEventProcessing(event: ProcessEventJobData['event']) {
  const jobData: ProcessEventJobData = { event };
  
  return processEventQueue.add(
...
export async function getQueueStats(queueName: string): Promise<JobStats | null> {
  const queue = getQueue(queueName);
...
export async function isClientTargetInSet(fid: number): Promise<boolean> {
  const result = await redisConnection.sismember('client-targets', fid.toString());
  return result === 1;
}
...
export async function initializeTargetSets() {
  try {
    ...
    const allTargets = await db.select({ fid: schema.targets.fid }).from(schema.targets);
    ...
    const allClientTargets = await db.select({ client_fid: schema.targetClients.clientFid }).from(schema.targetClients);
