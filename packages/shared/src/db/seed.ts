...
const sampleCasts = [
  ...
  },
  {
    ...
    parentHash:
      "cast1234567890abcdef1234567890abcdef1234567890abcdef1234567890",
    parentFid: 1,
...
const sampleReactions = [
  ...
  },
  {
    ...
    type: "recast",
    targetHash:
      "cast2234567890abcdef1234567890abcdef1234567890abcdef1234567890",
    targetFid: 2,
...
export async function seedDevelopmentData() {
  ...
  try {
    ...
    if (config.strategy.targetClients.length > 0) {
      ...
      await db
        .insert(schema.targetClients)
        .values(clientTargets)
        .onConflictDoNothing();
  ...
  try {
    const stats = await Promise.all([
      db
        .select()
        .from(schema.targets)
        .then((rows) => ({ table: "targets", count: rows.length })),
      db
        .select()
        .from(schema.targetClients)
        .then((rows) => ({ table: "target_clients", count: rows.length })),
      db
        .select()
        .from(schema.users)
      ...
      db
        .select()
        .from(schema.casts)
        .then((rows) => ({ table: "casts", count: rows.length })),
      db
        .select()
        .from(schema.reactions)
        .then((rows) => ({ table: "reactions", count: rows.length })),
      db
        .select()
        .from(schema.links)
        .then((rows) => ({ table: "links", count: rows.length })),
      db
        .select()
        .from(schema.verifications)
        .then((rows) => ({ table: "verifications", count: rows.length })),
      db
        .select()
        .from(schema.userData)
        .then((rows) => ({ table: "user_data", count: rows.length })),
      db
        .select()
        .from(schema.usernameProofs)
        .then((rows) => ({ table: "username_proofs", count: rows.length })),
      db
        .select()
        .from(schema.onChainEvents)
        .then((rows) => ({ table: "on_chain_events", count: rows.length })),
      db
        .select()
        .from(schema.syncState)
        .then((rows) => ({ table: "sync_state", count: rows.length })),
    ]);
