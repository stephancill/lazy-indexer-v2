...
export const casts = pgTable(
  ...
  (table) => ({
    ...
    fidTimestampIdx: index('casts_fid_timestamp_idx').on(table.fid, table.timestamp),
  }),
);
...
export const verifications = pgTable(
  'verifications',
  {
    ...
    protocol: varchar('protocol', { length: 50 }).notNull().default('ethereum'),
...
export const userData = pgTable(
  'user_data',
  {
    hash: varchar('hash', { length: 64 }).primaryKey(),
    fid: integer('fid').notNull(),
    type: varchar('type', { length: 50 }).notNull(), // 'pfp', 'display', 'bio', 'url', 'username'
...
export const onChainEvents = pgTable(
  ...
  (table) => ({
    ...
    blockNumberIdx: index('on_chain_events_block_number_idx').on(table.blockNumber),
    blockTimestampIdx: index('on_chain_events_block_timestamp_idx').on(table.blockTimestamp),
    transactionHashIdx: index('on_chain_events_transaction_hash_idx').on(table.transactionHash),
    chainIdBlockIdx: index('on_chain_events_chain_id_block_idx').on(table.chainId, table.blockNumber),
  }),
);
