...
describe('Database Schema', () => {
  ...
  describe('Targets Table', () => {
    it('should insert and retrieve targets', async () => {
      ...
      const retrieved = await testDb.select().from(schema.targets).where(eq(schema.targets.fid, 1));
    ...
    it('should handle target updates', async () => {
      ...
      await testDb.update(schema.targets)
  ...
  describe('Users Table', () => {
    it('should insert and retrieve users', async () => {
      ...
      const retrieved = await testDb.select().from(schema.users).where(eq(schema.users.fid, 1));
  ...
  describe('Casts Table', () => {
    it('should insert and retrieve casts', async () => {
      ...
      const retrieved = await testDb.select().from(schema.casts).where(eq(schema.casts.fid, 1));
    ...
    it('should handle cast replies', async () => {
      ...
      const replies = await testDb.select().from(schema.casts).where(eq(schema.casts.parentFid, 1));
  ...
  describe('Reactions Table', () => {
    it('should insert and retrieve reactions', async () => {
      const reaction = {
        ...
        fid: 1,
        type: 'like',
        targetHash: 'target1234567890abcdef1234567890abcdef1234567890abcdef1234567890',
        targetFid: 2,
        timestamp: new Date(),
      };
      ...
      const retrieved = await testDb.select().from(schema.reactions).where(eq(schema.reactions.fid, 1));
  ...
  describe('Links Table', () => {
    it('should insert and retrieve follows', async () => {
      ...
      const retrieved = await testDb.select().from(schema.links).where(eq(schema.links.fid, 1));
  ...
  describe('Verifications Table', () => {
    it('should insert and retrieve verifications', async () => {
      ...
      const retrieved = await testDb.select().from(schema.verifications).where(eq(schema.verifications.fid, 1));
      
      expect(retrieved).toHaveLength(1);
      expect(retrieved[0].address).toBe('0x1234567890abcdef1234567890abcdef12345678');
  ...
  describe('UserData Table', () => {
    it('should insert and retrieve user data', async () => {
      ...
      const retrieved = await testDb.select().from(schema.userData).where(eq(schema.userData.fid, 1));
  ...
  describe('OnChainEvents Table', () => {
    it('should insert and retrieve on-chain events', async () => {
      ...
      const retrieved = await testDb.select().from(schema.onChainEvents).where(eq(schema.onChainEvents.fid, 1));
  ...
  describe('SyncState Table', () => {
    it('should insert and retrieve sync state', async () => {
      ...
      const retrieved = await testDb.select().from(schema.syncState).where(eq(schema.syncState.name, 'realtime-sync'));
